...
import { urlBase, urlGenerarStickerEntrada } from "@/Utils/UrlData";


function FormStickerLogEntrada({stateClosedForm, propNumeroOficio}) {
    ...
    const { fetchPost, dataPost } = useFetchPost(token, `${urlBase}${urlGenerarStickerEntrada}`, dataDate);

    //funcion para cerrar el formulario que genera el sticker de entrada
    function handleClosedForm() {
        stateClosedForm(false);
    }

    //funcion para ejecutar el servicio POST para generar el sticker de entrada
    ...
    return (
        <FormStickers
            propTitleForm='Generar Sticker de Entrada'
...
export default FormStickerLogEntrada
